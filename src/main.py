...
class VisorView(ShowBase):
    ...
    def build_cog(self):
        ...
        #self.actor.find("**/joint_attachMeter").setH(180)

        self.shadow.reparentTo(self.actor.find('**/joint_shadow'))

        tx_suit = loader.loadTexture(globals.COG_DATA[self.current_cog]["suitTex"])
        ...
        if globals.COG_DATA[self.current_cog]["dept"] == "l":
            self.actor.find('**/necktie-s').hide()
            self.actor.find('**/necktie-w').hide()
        elif globals.COG_DATA[self.current_cog]["dept"] == "s":
            self.actor.find('**/necktie-w').hide()
        ...
        corp_medallion = icons.find('**/' + medallion).copyTo(iconbase)
        corp_medallion.setPosHprScale(*globals.COG_ICON_HPR)
        icons.setPosHprScale(*globals.COG_ICON_HPR)
        iconbase.setPosHprScale(*globals.COG_ICON_HPR)
        chest_null.setH(180)
        
        iconbase.find('**/emblem_hp').hide()
        iconbase.find('**/glow').hide()
        iconbase.find('**/emblem_sales').hide()
        iconbase.find('**/emblem_money').hide()
        iconbase.find('**/emblem_legal').hide()
        iconbase.find('**/emblem_corp').hide()
        iconbase.find('**/emblem_board').hide()
        
        
        if globals.COG_DATA[self.current_cog]["suit"] == "a":
            iconbase.setY(-0.10)
        elif globals.COG_DATA[self.current_cog]["suit"] == "c":
            iconbase.setY(0.10)
        else:
            iconbase.setY(0.00)
            
        if globals.COG_DATA[self.current_cog]["dept"] == "s":
            iconbase.find('**/emblem_sales').show()
        elif globals.COG_DATA[self.current_cog]["dept"] == "m":
            iconbase.find('**/emblem_money').show()
        elif globals.COG_DATA[self.current_cog]["dept"] == "l":
            iconbase.find('**/emblem_legal').show()     
        else:
            iconbase.find('**/emblem_corp').show()
        

        #chest_null.setH(0)

        head = loader.loadModel(globals.COG_DATA[self.current_cog]["head"])
        head.reparentTo(self.actor.find('**/joint_head'))
